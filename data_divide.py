...
def range3(jdata, ndata, jnum, nnum):
    ...
    for i in range(n):
        if col_diff[0,i] == 0:
            col_diff[0, i] = 1
    jdata = (jdata - col_min) / col_diff
    ndata = (ndata - col_min) / col_diff
    ...
    for i in range(nnum):
        for j in range(n):
            if ndata[i][0,j]+2 < 0:
                ...
                k = int(ndata[i][0,j]+3)
            num_sum[k] += 1
    xticks=["x<-2","-2<x<-1","-1<x<0","0<x<1","1<x<2","2<x"]
    pyplot.bar(range(6), num_sum, align='center')
    pyplot.xticks(range(6), xticks)
    pyplot.show()
    return jdata, ndata, jtabel, ntabel

def read_bigData(data_set, stand_method):
    jdata = []
    jtabel = []
    ndata = []
    ntabel = []
    ...
    file = open('dataSet/'+data_set+".data")
    fileLabels = open('dataSet/'+data_set+".labels")
    nn = 0
    for line in file:
        nn +=1
        adata = line.strip().split()
        adata = list(map(float, adata))
        try:
            label = int(fileLabels.readline().strip())
        except(ValueError):
            print(nn)
        if label > 0:
            jdata.append(adata)

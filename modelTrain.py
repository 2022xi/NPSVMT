...
def horner(n, R, sumy, sumyx, sumx, sumxx):
    ...
    # M = list(map(float, M))
    hend = time.time()
    # print("horner多项式压缩时间：",hend-hbegin)
    return M
...
def horner_1(n, R, M):
    ...
    hend = time.time()
    # print("horner多项式解压时间：",hend-hbegin)
...
def calculation(n, train_x, train_y, begin, end):
    # DPbegin = time.time()
    sumx = np.mat([0] * n).T
    ...
    R = int(max(R1,R2) * MULTIPLE)

    return R, sumy, sumyx, sumx, sumxx, sumy_1, sumyx_1, sumx_1, sumxx_1
...
def nonModel(train_x, train_y, m, clist):
    ...
    for i in range(0,m+1,m//10):
        ...
        R1, sumy, sumyx, sumx, sumxx, sumy_1, sumyx_1, sumx_1, sumxx_1 = calculation(n, train_x, train_y, begin, end)
        sumlist += [sumy, sumyx, sumx, sumxx, sumy_1, sumyx_1, sumx_1, sumxx_1]
        begin = end
        R += R1
    R = bin(R)
    R = len(R) - 2
    print("求参耗时：",time.time()-DPbegin)
    print("R=2^%d" % R)
    M = [public_key.encrypt(0) for _ in range(n+2)]
    M_1 = [public_key.encrypt(0) for _ in range(n+2)]
    for i in range(0,80,8):
        Add2(M, horner(n, R, sumlist[i], sumlist[i+1], sumlist[i+2], sumlist[i+3]))
        Add2(M_1, horner(n, R, sumlist[i+4], sumlist[i+5], sumlist[i+6], sumlist[i+7]))
    DPend = time.time()
